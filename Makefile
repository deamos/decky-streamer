...
zip: ## Create a zip file for sideloading via Decky (frontend only, uses system GStreamer)
	@echo "+ $@"
	@$(MAKE) build-front
	@rm -rf ./tmp/zip
	@mkdir -p ./tmp/zip/decky-streamer
	@cp -r ./dist ./tmp/zip/decky-streamer/
	@cp -r ./backend/out ./tmp/zip/decky-streamer/bin 2>/dev/null || echo "Note: backend/out not found - plugin will use system GStreamer"
	@cp ./main.py ./tmp/zip/decky-streamer/
	@cp ./settings.py ./tmp/zip/decky-streamer/
	@cp ./plugin.json ./tmp/zip/decky-streamer/
	@cp ./package.json ./tmp/zip/decky-streamer/
	@cd ./tmp/zip && zip -r ../decky-streamer.zip decky-streamer
	@echo -e "\n\033[0;32m✓ Created: tmp/decky-streamer.zip\033[0m"
	@echo -e "Upload this to your Steam Deck via Decky's settings menu\n"

zip-full: ## Create a zip with bundled GStreamer libs (requires Docker)
	@echo "+ $@"
	@$(MAKE) build
	@rm -rf ./tmp/zip
	@mkdir -p ./tmp/zip/decky-streamer
	@cp -r ./dist ./tmp/zip/decky-streamer/
	@cp -r ./backend/out ./tmp/zip/decky-streamer/bin
	@cp ./main.py ./tmp/zip/decky-streamer/
	@cp ./settings.py ./tmp/zip/decky-streamer/
	@cp ./plugin.json ./tmp/zip/decky-streamer/
	@cp ./package.json ./tmp/zip/decky-streamer/
	@cd ./tmp/zip && zip -r ../decky-streamer.zip decky-streamer
	@echo -e "\n\033[0;32m✓ Created: tmp/decky-streamer.zip (with bundled GStreamer)\033[0m"
	@echo -e "Upload this to your Steam Deck via Decky's settings menu\n"
